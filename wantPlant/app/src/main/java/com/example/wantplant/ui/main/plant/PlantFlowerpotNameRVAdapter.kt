...
class PlantFlowerpotNameRVAdapter (private val plantFlowerpotNameList: ArrayList<PlantFlowerpotName>): RecyclerView.Adapter<PlantFlowerpotNameRVAdapter.ViewHolder>() {
    ...
    inner class ViewHolder(val binding: ItemPlantFlowerpotNameBinding) : RecyclerView.ViewHolder(binding.root) {
        fun bind(plantFlowerpotName: PlantFlowerpotName, position: Int) {
            binding.itemPlantPotNameTv.text = plantFlowerpotName.flowerpotName
            if (position == selectedItemPosition) {
                binding.itemPlantPotNameTv.setBackgroundResource(R.drawable.border_nonfill_greenstroke_15radius)
                binding.itemPlantPotNameTv.setTextColor(Color.BLACK)

            } else {
                binding.itemPlantPotNameTv.setBackgroundResource(R.drawable.border_nonfill_graystroke_15radius)
                binding.itemPlantPotNameTv.setTextColor(Color.parseColor("#616161"))
            }
        }
    }
